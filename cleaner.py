...
def getEtfList(html):
    html=re.findall("<table class=\"dbtable\" id=\"oTable\">(.*?)</table>",html)[0]
...
def getHistList(content,callback,code):
    ...
    result=[]
    for item in dic:
        result.append((code,item['FSRQ'],item['DWJZ'],item['LJJZ'],
        item['SDATE'],item['ACTUALSYI'],item['NAVTYPE'],
        item['JZZZL'],item['SGZT'],item['SHZT'],
        item['FHFCZ'],item['FHFCBZ'],item['DTYPE'],
        item['FHSP']))
    return result    

def getAlbumIdList(html):
    html=re.findall("<div class=\"text _Vc\"><a title=\"(.*?)\" href=\"(.*?)\">",html)
    return html